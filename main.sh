...
tg_send_files(){
	...
	SID="CAACAgQAAxkBAAECc-9gz9TbsW8WSesewCEGXouWOcBo9gACJQEAAqghIQbYv0ET_Pm8eh8E"
...
CompileKernel(){
    ...
    if [ ! -z "$TAGKENEL" ];then
        export KBUILD_BUILD_HOST="KeepHealthy-$TAGKENEL"
    fi
	...
	if [[ ! -e $kernelDir/out/arch/$ARCH/boot/Image.gz-dtb ]];then
		...
		if [ "$branch" = "lynx-eas" ];then
         if [ $TypeBuild = "STABLE" ] || [ $TypeBuild = "RELEASE" ];then
            ...
            ZipName="[$KernelFor]$KVer-$KName-$TypeBuilder-$CODENAME.zip"
        else
            ZipName="[$KernelFor]$KVer-$KName-$TypeBuild-$TypeBuilder-$CODENAME.zip"
        fi
